...
# -----------------------------
# ORTAM 3 ASCII LABİRENT - 4 KAPI / 4 BUTON
# -----------------------------
maze_lines = [
    "########################################",
    "#           #          #               #",
...
actionsW = np.array([-0.30, -0.15, -0.05, 0.0, 0.05, 0.15, 0.30], dtype=float)
nA = len(actionsW)
...
for ep in range(1, maxEpisodes + 1):
    ...
    for step in range(1, max_steps + 1):
        ...
        if wait_for_door:
            ...
            v = float(v_ref)
            # FİZİKSEL ZORUNLULUK: w_max 0.30 iken dar labirentte dönebilmek için viraj freni
            if abs(theta_err_ctrl) > 0.4:
                v *= 0.25

            w_raw = float(actionsW[a])
            prev_w_old = prev_w
            w = 0.88 * prev_w_old + 0.12 * w_raw
...
# Çıktı klasörü
# -----------------------------
output_dir = r"C:\Users\zulal\OneDrive\Masaüstü\mobilrobot\RL\ORTAM-3\output-03"
...
plt.title("A* + RL Hibrit Eğitim: Toplam Ödül (Ortam-3)")
...
plt.title("A* + RL Hibrit Eğitim: Adım Sayısı (Ortam-3)")
plt.grid(True, linestyle="--", alpha=0.7)
...
plt.title("A* + RL Hibrit Eğitim: Başarı Oranı (Ortam-3)")
plt.ylim(-0.05, 1.05)
plt.grid(True, linestyle="--", alpha=0.7)
plt.legend(loc="upper right", fontsize="small")
plt.tight_layout()
...
ax_tr.set_title("A* + RL Mobil Robot Eğitim İzleri (Ortam-3)", color="white", fontsize=16, pad=20)
...
ax.set_title("A* + RL Mobil Robot Yol Planlama (Ortam-3)", color="w", fontsize=14, pad=20)
