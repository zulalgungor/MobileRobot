...
def force_free_around(occ: np.ndarray, idx_xy: tuple[int, int], r: int) -> np.ndarray:
    """
    Gridde start/goal çevresini zorla boşalt.
    idx_xy: (x_index, y_index)
    """
    occ2 = occ.copy()
    ny, nx = occ2.shape
    cx, cy = idx_xy
...
def astar_grid_with_doors(
    occ: np.ndarray,
    door_grid: np.ndarray,
    ...
    """
    Kapı-buton mantıklı A*.
    state = (x, y, mask)
    mask: hangi butonların aktifleştiğini tutar.
    """
    ny, nx = occ.shape
    sx, sy = start_xy
    gx, gy = goal_xy
...
# -----------------------------
# ORTAM 2 ASCII LABİRENT - 3 KAPI / 3 BUTON
# -----------------------------
maze_lines = [
    "########################################",
    "#        #         #         #         #",
...
def parse_ascii_maze(lines, resolution):
    ny = len(lines)
    nx = max(len(line) for line in lines)
    ...
    if start_xy is None:
        raise ValueError("Labirentte başlangıç noktası 'A' bulunamadı.")
    if goal_xy is None:
        raise ValueError("Labirentte hedef noktası 'Z' bulunamadı.")
    if len(button_xy_list) == 0:
        raise ValueError("Labirentte buton 'B' bulunamadı.")

    door_coords_list = [[] for _ in button_xy_list]
...
print(
    f"Başlangıç: ({start[0]:.2f}, {start[1]:.2f}), "
    f"Hedef: ({goal[0]:.2f}, {goal[1]:.2f}), "
    f"Buton sayısı: {len(button_idx_list)}"
)
...
path_data = astar_grid_with_doors(
    occ_inflated,
    door_grid,
    start_idx,
    goal_idx,
...
if len(path_data) == 0:
    raise RuntimeError("Yol bulunamadı! Kapı-buton eşleşmesi veya labirent geçişleri kontrol edilmeli.")
...
# A* yolunda buton/kapı maskesinin değiştiği noktalar.
door_trigger_info = []
prev_m = 0
...
print("Kapı tetikleme noktaları:", door_trigger_info)


# =========================================================
# A* + RL HİBRİT EĞİTİM / TEST
# =========================================================
dt = 0.2
v_ref = 0.40
look_ahead_val = 20
...
actionsW = np.array([-0.20, -0.10, -0.04, 0.0, 0.04, 0.10, 0.20], dtype=float)
nA = len(actionsW)
...
def segment_collision_with_doors(x1, y1, x2, y2, occ, door_grid, mask, res, samples=7):
    """Robot genişliğini yaklaşık hesaba katan çarpışma kontrolü."""
    ny, nx = occ.shape
    check_radius = 0.3
    ...
    for i, (bx, by) in enumerate(button_idx_list):
        ...
        if dist_to_btn < button_thresh:
            ...
            if door_progress[i] < 1.0:
                door_progress[i] = min(1.0, door_progress[i] + 0.15)
...
for ep in range(1, maxEpisodes + 1):
    ...
    # İyi ep'leri koru hack'i (bir önceki bölüm başarılıysa yavaş öğren)
    if len(epSuccess) > 0 and epSuccess[-1] == 1:
        alpha_use = 0.1
    else:
        alpha_use = 0.3
    ...
    for step in range(1, max_steps + 1):
        ...
        target = get_active_target(rx, ry, door_progress, look_ahead=look_ahead_val)
        ...
        if wait_for_door:
            ...
            theta_err_ctrl = wrap_to_pi(th_des - th)
            
            v = float(v_ref)
            # FİZİKSEL ZORUNLULUK: w_max 0.20 iken dar labirentte dönebilmek için viraj freni (dönüş yarıçapını 2.0m'den 0.5m'ye indirir)
            if abs(theta_err_ctrl) > 0.4:
                v *= 0.25

            w_raw = float(actionsW[a])
            prev_w_old = prev_w
            w = 0.92 * prev_w_old + 0.08 * w_raw
            
            if abs(theta_err_ctrl) < 0.05:
                w = 0.0
        ...
        target2 = get_active_target(rx2, ry2, door_progress, look_ahead=look_ahead_val)
        ...
        goal_dist_now = np.hypot(rx - goal[0], ry - goal[1])
        goal_dist_new = np.hypot(rx2 - goal[0], ry2 - goal[1])

        if wait_for_door:
            r = waitingPenalty + r_extra
        else:
            r = 3.0 * (d_now - d_new) + stepPenalty + r_extra
            r += 3.0 * (goal_dist_now - goal_dist_new)  # Goal shaping geri eklendi ve güçlendirildi
            ...
            r += -0.25 * abs(theta_err)
            r += -0.40 * abs(w - prev_w_old)
            r += 20.0 * (dist_to_path - dist_to_path2)

            if dist_to_path2 > 0.25:
                r -= 0.05
            else:
                r += 0.05

            if d_min < dDanger:
                r += dangerPenalty * (1.0 - d_min / dDanger)
    ...
    if ep % 20 == 0:
        print(
            f"Ep {ep} | Return={totalR:.1f} | Steps={step} | "
            f"RecentSR={recentSR:.2f} | eps={epsilon:.2f} | Q-size={len(Q)}"
        )
...
for step in range(1, max_steps + 1):
    ...
    sim_mask = get_open_mask_from_progress(door_progress)
    _, wait_for_door = update_button_and_door_progress(rx, ry, door_progress, button_used)
    sim_mask2 = get_open_mask_from_progress(door_progress)
...
print("Simülasyon bitti.", f"Hedefe ulaşıldı: {reached_step is not None}, adım={reached_step}")


# -----------------------------
# Çıktı klasörü
# -----------------------------
output_dir = r"C:\Users\zulal\OneDrive\Masaüstü\mobilrobot\RL\ORTAM-2\output-02"
os.makedirs(output_dir, exist_ok=True)


# =========================================================
# Eğitim metrik grafikleri
# =========================================================
...
if len(epReturn_arr) >= win_r:
    ...
    plt.plot(np.arange(win_r - 1, win_r - 1 + len(mov_return)), mov_return, linewidth=2.2, label=f"{win_r} Ep. Ort.")
plt.xlabel("Episode")
plt.ylabel("Toplam Ödül")
plt.title("A* + RL Hibrit Eğitim: Episode Return (Ortam-2)")
...
if len(epSteps_arr) >= win_s:
    ...
    plt.plot(np.arange(win_s - 1, win_s - 1 + len(mov_steps)), mov_steps, linewidth=2.2, label=f"{win_s} Ep. Ort.")
plt.xlabel("Episode")
plt.ylabel("Adım Sayısı")
plt.title("A* + RL Hibrit Eğitim: Episode Steps (Ortam-2)")
plt.grid(True, linestyle="--", alpha=0.7)
...
if len(epSuccess_arr) >= win_m:
    ...
    plt.plot(np.arange(win_m - 1, win_m - 1 + len(mov_success)), mov_success, linewidth=2.0, label=f"{win_m} Ep. Ort.")
else:
    plt.plot(epSuccess_arr, linewidth=1.5, label="Ep. Success")
plt.xlabel("Episode")
plt.ylabel("Başarı Oranı")
plt.title("A* + RL Hibrit Eğitim: Başarı Oranı (Ortam-2)")
plt.ylim(-0.05, 1.05)
plt.grid(True, linestyle="--", alpha=0.7)
plt.legend(loc="upper right", fontsize="small")
plt.tight_layout()
...
ax_tr.set_xlim(0, envW)
ax_tr.set_ylim(0, envH)
ax_tr.set_xlabel("X (m)", color="white", fontsize=12)
ax_tr.set_ylabel("Y (m)", color="white", fontsize=12)
ax_tr.set_title("A* + RL Mobil Robot Eğitim İzleri (Ortam-2)", color="white", fontsize=16, pad=20)
...
if len(training_paths) > 1000:
    filtered_paths = training_paths[:500] + training_paths[-500:]
else:
    filtered_paths = training_paths

for ep_num, tx, ty, succ in filtered_paths:
    color = sm.to_rgba(ep_num)
    alpha = 0.4 if succ else 0.1
...
# Animasyon
# =========================================================
fig = plt.figure(figsize=(10, 8), facecolor=room_bg)
ax = plt.gca()
...
ax.set_title("A* + RL Mobil Robot Yol Planlama (Ortam-2)", color="w", fontsize=14, pad=20)
...
ys, xs = np.where(occ_base)
for yy, xx in zip(ys, xs):
    ax.add_patch(Rectangle((xx * res, yy * res), res, res, facecolor=wall_col, edgecolor=wall_col, alpha=0.9))
...
robot_marker, = ax.plot([start[0]], [start[1]], marker="o", color=path_col, markerfacecolor=path_col, markersize=8)
...
gif_path = os.path.join(output_dir, "ORTAM2.gif")
...
plt.title("Hedefe Mesafe vs Zaman (Ortam-2)")
plt.grid(True, linestyle="--", alpha=0.7)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "metric_distance.png"), dpi=150)
plt.show()
...
plt.xlabel("Zaman (s)")
plt.ylabel("Hata (rad)")
plt.title(r"Heading Hatası ($\theta_{err}$) vs Zaman (Ortam-2)")
plt.grid(True, linestyle="--", alpha=0.7)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "metric_heading.png"), dpi=150)
plt.show()
...
plt.title("Kontrol Girişleri: v ve w (Ortam-2)")
plt.grid(True, linestyle="--", alpha=0.7)
plt.legend(loc="upper right", fontsize="small")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "metric_controls.png"), dpi=150)
plt.show()
...
path_length = np.sum(np.sqrt(np.diff(xs_log) ** 2 + np.diff(ys_log) ** 2)) if len(xs_log) > 1 else 0.0
...
max_theta_err = np.max(np.abs(therr_log)) if len(therr_log) > 0 else 0.0
